...
export const APP_RELEASE = Object.freeze({
  ...
  releaseHighlights: [
    'Installed app shell now stays focused on ordering by removing download-app prompts, links, and app-download detours for existing users',
...
const normalizeAppRelease = (release = {}) => ({
  ...
  minimumSupportedBuild:
    Number.parseInt(release.minimumSupportedBuild ?? APP_RELEASE.minimumSupportedBuild, 10) ||
    APP_RELEASE.minimumSupportedBuild,
  minimumSupportedVersion: String(
    release.minimumSupportedVersion || APP_RELEASE.minimumSupportedVersion,
  ),
  forceUpdate: Boolean(release.forceUpdate ?? APP_RELEASE.forceUpdate),
...
export const isAppForceUpdateRequired = (
  currentVersion,
  currentBuild = 0,
  release = APP_RELEASE,
) => {
  ...
  if (normalizedRelease.forceUpdate) {
    return true;
  }

  const build = Number.parseInt(currentBuild, 10) || 0;

  if (build < normalizedRelease.minimumSupportedBuild) {
    return true;
  }

  return compareAppVersions(currentVersion, normalizedRelease.minimumSupportedVersion) < 0;
};
